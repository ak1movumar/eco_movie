...
export async function generateMetadata({
  ...
  const description = movie.overview || `Watch ${movie.title}, a ${movie.genres?.map((g: any) => g.name).join(", ") || "movie"} released in ${movie.release_date?.split("-")[0] || ""}.`;
...
export default async function Page({ 
  ...
  const structuredData = data && data.movie ? {

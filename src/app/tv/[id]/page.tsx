...
export async function generateMetadata({
  ...
  const description = tv.overview || `Watch ${tv.name}, a ${tv.genres?.map((g: any) => g.name).join(", ") || "TV show"} that first aired in ${tv.first_air_date?.split("-")[0] || ""}.`;
...
export default async function Page({ 
  ...
  const structuredData = data && data.tv ? {
